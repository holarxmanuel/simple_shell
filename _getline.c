...
/**
 * salida3 - nrmal out
 * @m: copy o environmental variables
 * @e: numberof elements in m
 */
...
/**
 * salida2 - ut with double Ctrl+D
 *@m: copy ofenvironmental variables
...
/**
 * getline - function to read what the user writes
 * @a: pointr to loop counter
 * @e: lengt of m
 * @m: copy f environmental
 * Return: le in sucess otherwise NULL.
 */
char  *getline(int *a, char **m, int e)
{
	...
	for (; (num != 0); bufsize = 0, free(line))
	{
		...
		*a = *a + 1;
		...
		{
			if (bufsize == 0)
				line = calloc(bufsize + 3, sizeof(char));
			else
				line = realloc(line, bufsize, bufsize + 3);
			if (!line)
			{
				num = 0;
				break;
...
char  *getlineav(int *a, char **m, int e, char **av)
{
	...
	for (; (num != 0); bz = 0, free(li))
	{
		...
		*a = *a + 1;
		...
		{
			if (bz == 0)
				li = calloc(bz + 3, sizeof(char));
			else
				li = realloc(li, bz, bz + 3);
			if (!li)
			{
				num = 0;
				break;
